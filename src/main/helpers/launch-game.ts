...
import { launchGameInVM as launchGameInVMService } from "../services/vm-launcher";
...
/**
 * Thin wrapper around the VM launcher service so the event layer stays clean.
 * Launches the given game executable inside an isolated VM.
 */
export const launchGameInVM = async (
  options: LaunchGameOptions
): Promise<void> => {
  await launchGameInVMService({
