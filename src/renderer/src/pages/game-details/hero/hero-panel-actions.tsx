...
import { Button, ConfirmationModal, VMLaunchModal } from "@renderer/components";
...
export function HeroPanelActions() {
  ...
  const [showVMLaunchModal, setShowVMLaunchModal] = useState(false);
  ...
  const isHeavyGame = (game: Game | null): boolean => {
    if (!game) return false;
    // Games released after 2023 or with known heavy titles are flagged
    // Uses shopDetails?.release_date.date if available
    const releaseDate = shopDetails?.release_date?.date ?? null;
    const releaseYear = releaseDate
      ? new Date(releaseDate).getFullYear()
      : null;
    if (releaseYear && releaseYear >= 2024) return true;
    const heavyTitles = [
      "cyberpunk",
      "hogwarts",
      "starfield",
    ...
    return heavyTitles.some((t) => titleLower.includes(t));
  };

  const handleLaunchInVM = async () => {
    setShowVMLaunchModal(false);
    if (!game) return;
  ...
  const handleLaunchOnHost = async () => {
    setShowVMLaunchModal(false);
    await openGame();
  };
  ...
  const gameActionButton = () => {
    ...
    if (game?.executablePath || isPlayableClassics) {
      return (
        <Button
          onClick={() => setShowVMLaunchModal(true)}
          theme="outline"
  ...
  if (game) {
    return (
      <div className="hero-panel-actions__container">
        <VMLaunchModal
          visible={showVMLaunchModal}
          onClose={() => setShowVMLaunchModal(false)}
          onLaunchInVM={handleLaunchInVM}
          onLaunchOnHost={handleLaunchOnHost}
          isHeavyGame={isHeavyGame(game)}
          gameName={gameTitle ?? ""}
        />
        {gameActionButton()}
